---
title: "Sagebrush Biome Tree Cover Change and Rangeland Productivity Reports"
...
> *"Woodland expansion into grasslands and shrublands is a global problem as trees displace rangeland wildlife and reduce productivity of grazing lands. Scattered young trees in sagebrush country may look harmless to a casual observer, but science shows conifer expansion can erode rangeland resilience if left unchecked."*
>
> `r tufte::quote_footer('--- Sagebrush Biome Framework for Conservation Action')`
...
biome_sf <- st_read('C:/Users/eric/Desktop/CountyForage/data/shp/sagebrushBoundary/sagebrushBoundary.shp', quiet = TRUE) %>% 
  st_transform(4326) %>%
  st_simplify(dTolerance = 0.02)
...
leaflet(options = leafletOptions(zoomControl = FALSE, zoomSnap=0.01, zoomDelta = 0.25, minZoom = 5.25, maxZoom = 5.25, dragging = FALSE, attributionControl=FALSE)) %>%
  ...
  addLabelOnlyMarkers(data = biome_stas_cent_sf,
                      ...
                      labelOptions = leaflet::labelOptions(noHide = TRUE,
                                                           ...
                                                           "font-weight" =  "bold",
                                                           "font-family" = "Open sans"))) %>%
  addLegend('bottomleft', color = make_shapes('#328C1D', 15, '#328C1D', 'empty_square'), labels = 'Sagebrush Biome') %>%
  setMapWidgetStyle(list(background= "white"))  %>%
  htmlwidgets::onRender(jsOnClick) %>%
  addProviderTiles(providers$Esri.WorldTerrain) %>%
  setView(lng = -111, lat = 41, zoom = 5.5) %>%
...
## Reference:
* [See the reports for the Great Plains Biome](../greatplains/index.html)  
