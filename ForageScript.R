...
for (i in stas_fp_v[1]) {
  ...
  sta_ctys_sf <- ctys_sf %>%
    filter(STATEFP == i) %>%
    mutate(
      up_path = paste('../', str_replace_all(NAME, ' ', ''), '/index.html', sep = '') %>% stringr::str_replace_all("ñ", "n"),
  ...
  char_html <- char_df %>%
    tableHTML(
      widths = c(300, 200),
      rownames = FALSE,
      caption = paste('Analysis area summary for', sta_name)
    ) %>%
    add_css_row(css = list(
      c(
        'background-color',
  ...
  sta_rast <-
    raster(
      paste(
        'C:/Users/eric/Documents/NTSG/Projects/RAP/Scripts/CountyForage/data/tif/v6/240m_median/',
  ...
  aspect_factor <-
    (max(sta_rast_df$x) - min(sta_rast_df$x)) / (max(sta_rast_df$y) - min(sta_rast_df$y))
  ...
  for (j in sta_ctys_v[1]) {
    ...
    char_html <- char_df %>%
      tableHTML(
        widths = c(300, 200),
        rownames = FALSE,
        caption = paste('Analysis area summary for', loc_name)
      ) %>%
      add_css_row(css = list(
        c(
          'background-color',
